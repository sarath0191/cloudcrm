// Extend the User class to include the storeName attribute class User {
    private String username;
    private String password;
    private Set<String> roles;
    private String storeName; // Add a new attribute for the store name
    public User(String username, String password, Set<String> roles, String storeName) { this.username = username;
    ...
    auth.addUser(admin); auth.addUser(manager); auth.addUser(store6);
    // Simulate login as "Store 6"
    User loggedInUser = auth.login("store6", "store6123");
    if (loggedInUser != null) {
    String storeName = loggedInUser.getStoreName();
    if (!storeName.isEmpty()) { System.out.println("Logged in as " + storeName); // Implement store-specific actions for "Store 6"
    } else {
